...
import net.consular.cataclysm.block.ModLandingBlock;
...
public class ModFallingBlockEntity extends Entity {
    ...
    protected static final TrackedData<BlockPos> BLOCK_POS = DataTracker.registerData(ModFallingBlockEntity.class, TrackedDataHandlerRegistry.BLOCK_POS);

    public ModFallingBlockEntity(EntityType<? extends ModFallingBlockEntity> entityType, World world) {
        super(entityType, world);
    }

    public ModFallingBlockEntity(World world, double x, double y, double z, BlockState block) {
        this((EntityType<? extends ModFallingBlockEntity>)ModEntities.MOD_FALLING_BLOCK, world);
        this.block = block;
    ...
    public static ModFallingBlockEntity spawnFromBlock(World world, BlockPos pos, BlockState state) {
        ModFallingBlockEntity fallingBlockEntity = new ModFallingBlockEntity(world, (double)pos.getX() + 0.5, pos.getY(), (double)pos.getZ() + 0.5, state.contains(Properties.WATERLOGGED) ? (BlockState)state.with(Properties.WATERLOGGED, false) : state);
    ...
    public void tick() {
        ...
        this.getWorld().getOtherEntities(this, this.getBoundingBox(), predicate).forEach(entity -> entity.damage(entity.getDamageSources().generic(), 2));
                    ...
                    if (!this.destroyedOnLanding) {
                        ...
                        if (bl3 && bl5) {
                            ...
                            if (this.getWorld().setBlockState(blockPos, this.block, Block.NOTIFY_ALL)) {
                                ...
                                this.discard();
                                if (block instanceof ModLandingBlock) {
                                    ((ModLandingBlock)((Object)block)).onLanding(this.getWorld(), blockPos, this.block, blockState, this);
                                }
    ...
    public void onDestroyedOnLanding(Block block, BlockPos pos) {
        if (block instanceof ModLandingBlock) {
            ((ModLandingBlock)((Object)block)).onDestroyedOnLanding(this.getWorld(), pos, this);
        }
    }
    ...
    public boolean handleFallDamage(float fallDistance, float damageMultiplier, DamageSource damageSource) {
        ...
        Block block = this.block.getBlock();
        if (block instanceof ModLandingBlock) {
            ModLandingBlock ModLandingBlock = (ModLandingBlock)((Object)block);
            damageSource2 = ModLandingBlock.getDamageSource(this);
        } else {
